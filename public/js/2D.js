...
Intersection.intersectLineLine=function(a1,a2,b1,b2){
	...
	if(u_b!=0){
		...
		if (0.01<ua&&ua<0.99&&0.01<ub&&ub<0.99){
			result=new Intersection("Intersection");
			result.points.push({
				coords: [a1.x+ua*(a2.x-a1.x),a1.y+ua*(a2.y-a1.y)],
				relative: ua,
				relative2: ub,
				intersect: true
			});
		} else if ((((-0.01 < ua && ua < 0.01) || (0.99 < ua && ua < 1.01)) && (-0.01 < ub && ub < 1.01)) ||
			((-0.01 < ub && ub < 0.01) || (0.99 < ub && ub < 1.01)) && (-0.01 < ua && ua < 1.01)) {
			// ignored by pattern meta-data generation
			// but used for cropping patterns
			result = new Intersection("Close Intersection");
			result.points2 = [];
			result.points2.push({
				coords: [a1.x+ua*(a2.x-a1.x),a1.y+ua*(a2.y-a1.y)],
				relative: ua,
...
Pattern.prototype = new Path();
Pattern.prototype.constructor = Path;
Pattern.superclass = Path.prototype;
function Pattern(p) { this.init(p); }
Pattern.prototype.init = function(p) {
	Shape.prototype.init.call(p);
	this.parseData(p.line);
}
Pattern.prototype.intersectShape = function(shape){
	var result=new Intersection("No Intersection");
	for(var i=0;i<this.segments.length;i++){
		var inter=Intersection.intersectShapes(this.segments[i],shape);
		if (shape.getIntersectionParams().name === "Path") {
			if (inter.points.length > 0) {
				result.appendPoint(inter.points);
			} else {
				result.appendPoint([]);
			}
		} else {
